...
class IAMDataset(Dataset):
    ...
    def prepocess_data(self):
        data_path_list = self._create_data_path_list()
        text_array = []
        strokes_array = []

        for ascii_file, strokes_files in data_path_list:
            # Get the text from the files
            text_list = self._get_ascii(ascii_file)

            # Get the strokes from the files
            strokes_list = self._get_strokes(strokes_files)

            for text, strokes in zip(text_list, strokes_list):
                if len(text) > 10:
                    text_array.append(text)
                    strokes_array.append(self._convert_stroke_to_array(strokes))
                else:
    ...
    def _create_data_path_list(self):
        data_path_list = []
    ...
    @staticmethod
    def _get_strokes(filename_list):
        result = []
    ...
    @staticmethod
    def _convert_stroke_to_array(stroke):
        n_point = 0
